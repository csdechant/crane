//A linear interpolation version of CRANE's ADZapdosEEDFRateConstant

#include "ADZapdosEEDFRateLinearInterpolation.h"
#include "MooseUtils.h"

// MOOSE includes
#include "MooseVariable.h"

registerADMooseObject("CraneApp", ADZapdosEEDFRateLinearInterpolation);

InputParameters
ADZapdosEEDFRateLinearInterpolation::validParams()
{
...
ADZapdosEEDFRateLinearInterpolation::ADZapdosEEDFRateLinearInterpolation(const InputParameters & parameters)
    ...
    _mean_en(isCoupled("mean_energy") ? adCoupledValue("mean_energy") : _em)
{
  ...
  _coefficient_interpolation = libmesh_make_unique<LinearInterpolation>(val_x, rate_coefficient, true);
}

void
ADZapdosEEDFRateLinearInterpolation::computeQpProperties()
{
  ...
  _rate_coefficient[_qp].derivatives() =
      _coefficient_interpolation->sampleDerivative(std::exp(_mean_en[_qp].value() - _em[_qp].value())) *
