#include "ZapdosEEDFRateLinearInterpolation.h"
#include "MooseUtils.h"

// MOOSE includes
#include "MooseVariable.h"

registerMooseObject("CraneApp", ZapdosEEDFRateLinearInterpolation);

template <>
InputParameters
validParams<ZapdosEEDFRateLinearInterpolation>()
{
...
ZapdosEEDFRateLinearInterpolation::ZapdosEEDFRateLinearInterpolation(const InputParameters & parameters)
  : Material(parameters),
    ...
    _mean_en(isCoupled("mean_energy") ? coupledValue("mean_energy") : _zero)
{
  if (!isParamValid("sampler") && !isParamValid("mean_energy"))
    mooseError("Material ZapdosEEDFRateLinearInterpolation requires either a sampling variable or the "
...
void
ZapdosEEDFRateLinearInterpolation::computeQpProperties()
{
